...
export const AddItemInCart = (CartItems) => async (dispatch, getState) => {
    ...
    try {
        ...
        const response = await axios.post(
            'https://www.doggy.adsdigitalmedia.com/api/v1/Product/Add-Item',
            CartItems,
            {
                headers: {
...
export const getItemOfCart = () => async (dispatch, getState) => {
    ...
    try {
        const response = await axios.get('https://www.doggy.adsdigitalmedia.com/api/v1/Product/Get-Items', {
            headers: {
                Authorization: `Bearer ${token}`,
            },
        });
...
export const RemoveItem = (ProductId, selectedPackId) => async (dispatch, getState) => {
    const token = selectToken(getState());
    try {
        await axios.delete('https://www.doggy.adsdigitalmedia.com/api/v1/Product/Remove-Item', {
            headers: {
                Authorization: `Bearer ${token}`,
            },
            data: {
...
export const RemoveAllItem = () => async (dispatch, getState) => {
    const token = selectToken(getState());
    try {
      const res =   await axios.delete('https://www.doggy.adsdigitalmedia.com/api/v1/Product/Remove-All-Item', {
            headers: {
                Authorization: `Bearer ${token}`,
            },
        });
...
export const UpdateItemQuantityOfCart = (ProductId, Quantity) => async (dispatch, getState) => {
    const token = selectToken(getState());
    try {
        await axios.patch('https://www.doggy.adsdigitalmedia.com/api/v1/Product/Update-Quantity', {
            ProductId,
            Quantity,
        }, {
            headers: {
