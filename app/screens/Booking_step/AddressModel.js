...
export default function AddressModel({ isOpen, onClose, type,onSelectAddress  }) {
    ...
    const handleGetAddress = async () => {
        ...
        console.log(token)
        try {
            const { data } = await axios.get('https://www.doggy.adsdigitalmedia.com/api/v1/pet/get-address', {
                headers: {
    ...
    const handleAddAddress = async () => {
        setLoading(true);
        try {
            const { data } = await axios.post('https://www.doggy.adsdigitalmedia.com/api/v1/pet/add-address', addressData, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
    ...
    const handleRemoveAddress = async (id) => {
        try {
            const { data } = await axios.delete(`https://www.doggy.adsdigitalmedia.com/api/v1/pet/remove-address/${id}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
