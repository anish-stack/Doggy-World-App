...
exports.SingleVoucher = async (req, res) => {
    try {
        const { id } = req.query
        console.log(id)
        const fetchSingle = await Voucher.findById(id)
        // if (fetchSingle.length === 0) {
        //     return res.status(403).json({
        //         success: false,
        //         msg: "No Vouchers"
        //     })
...
// Toggle a voucher's active status
exports.toggleVoucher = async (req, res) => {
    try {
        ...
        const updatedVoucher = await Voucher.findByIdAndUpdate(id, { Active: !voucher.Active }, { new: true });
